    ...
    context 'with a query' do
      context 'with a valid search' do
        ...
        it 'calls the serializer with the correct arguments' do
          expect(PageSerializer::SearchPage::ResultsPageSerializer).to have_received(:new).with(query: 'banana', results: [1, 2, 3], pagination_hash: 'pagination_hash', request_id: '|1234abcd.')
        end
      end
      ...
      context 'an invalid search' do
        ...
        it 'calls the serializer with the correct arguments' do
          ...
          expect(PageSerializer::SearchPage::ResultsPageSerializer).to have_received(:new).with(query: 'fdsfsd', results: an_instance_of(Feedjira::Parser::Atom), pagination_hash: pagination_hash, request_id: '|1234abcd.')
        end
      end
      ...
      context 'with an empty string' do
        ...
        it 'calls the serializer with the correct arguments' do
          expect(PageSerializer::SearchPage::LandingPageSerializer).to have_received(:new).with(flash_message: I18n.t('search_controller.index.flash'), request_id: '|1234abcd.')
        end
      end
      ...
      context 'rescuing from errors' do
        ...
        it 'when making the search request' do
          ...
          expect(PageSerializer::SearchPage::ResultsPageSerializer).to have_received(:new).with(query: 'Allan Wazacz', request_id: '|1234abcd.' )
        end
      end
    end
  end
