...
RSpec.describe Houses::MadeAvailableController, vcr: true do
  describe 'GET index' do
    let(:data_alternates) do
      [{
         href: "#{ENV['PARLIAMENT_BASE_URL']}/house_laid_papers.nt?house_id=1AFu55Hs",
         type: 'application/n-triples'
       },
       { href: "#{ENV['PARLIAMENT_BASE_URL']}/house_laid_papers.ttl?house_id=1AFu55Hs",
         type: 'text/turtle' },
       { href: "#{ENV['PARLIAMENT_BASE_URL']}/house_laid_papers.tsv?house_id=1AFu55Hs",
         type: 'text/tab-separated-values' },
       { href: "#{ENV['PARLIAMENT_BASE_URL']}/house_laid_papers.csv?house_id=1AFu55Hs",
         type: 'text/csv' },
       { href: "#{ENV['PARLIAMENT_BASE_URL']}/house_laid_papers.rj?house_id=1AFu55Hs",
         type: 'application/json+rdf' },
       { href: "#{ENV['PARLIAMENT_BASE_URL']}/house_laid_papers.json?house_id=1AFu55Hs",
         type: 'application/json+ld' },
       { href: "#{ENV['PARLIAMENT_BASE_URL']}/house_laid_papers.xml?house_id=1AFu55Hs",
    ...
    context 'calling the serializers correctly' do
      it 'calls the Heading1ComponentSerializer correctly' do
        expect(ComponentSerializer::Heading1ComponentSerializer).to have_received(:new).with(heading: 'houseName - 1 - made available', subheading: 'houseName - 1', subheading_link: '/houses/1AFu55Hs')
      end
